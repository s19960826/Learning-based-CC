...
void 
MyOnOffHelper::SetHelpController (Ptr<ControlDecider> controller)
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
	  ...
	  //onoff->SetOnOffController(help_controller);
          //std::cout << "this is" << help_controller->avgThroughput << std::endl;
          if (onoff)
            {
