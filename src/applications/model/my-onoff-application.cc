...
MyOnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MyOnOffApplication")
    ...
    .AddAttribute ("Interval", "The time between packets sent in on state",
                   ...
                   MakePointerChecker <RandomVariableStream>())   //adding the interval value
    .AddAttribute ("tuple", "five tuple of each application.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MyOnOffApplication::app_tuple),
...
//set onoff controller
void 
MyOnOffApplication::SetTuple (uint32_t tuple)
{
  app_tuple = tuple;
}
...
void MyOnOffApplication::StartApplication () // Called at time specified by Start
{
  ...
  ScheduleStartEvent ();
  //std::cout << "this is" << onoff_controller->avgThroughput << std::endl;
}
